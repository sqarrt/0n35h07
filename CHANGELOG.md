...
### Fixed
- Десктоп ведёт себя как 0.5.0 (старая версия в углу, старое положение прицела) даже после пересборки
  фронта — главная причина: PWA-service worker. В WebView2 хранилище привязано к идентификатору приложения,
  и SW от прошлой 0.5.0-сборки персистентно кэшировал ассеты и отдавал их, сколько `dist` ни пересобирай.
  В Tauri-сборке PWA теперь самоуничтожается (`selfDestroying`, детект по `TAURI_ENV_PLATFORM`): снимает
  регистрацию старого SW и чистит кэши WebView2; в вебе обычный PWA сохранён.
