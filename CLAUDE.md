...
Меню/лобби — машина состояний экранов в `App` (menu/join/lobby/game) + hash-роутинг. Лобби **строго 1v1, всегда
p2p**: создатель кода = хост (`HOST_ID=0`), вход по `#CODE` = клиент. `LobbySession` держит `hostEntry` + ОДИН
