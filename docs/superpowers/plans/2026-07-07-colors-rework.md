...
- [ ] **Step 6: Отложенные правки, найденные на ревью ветки (НЕ регрессии ветки)**

Обнаружено при пользовательском тестировании 2026-07-07; согласовано отложить и вернуться:

1. **Лобби-задник теряет ballArt (баг с 0.5.8, ветка не виновата).** `MenuBackdrop.tsx` `specOf`/`computeBalls` в режиме `lobby` строят спек шара из ростера БЕЗ `ballArt` → при переходе из главного меню на вкладки PLAY / «С другом» / «С ботом» тот же StageBall получает `setArt(null)` и рисунок исчезает (в главном меню и Appearance рисунок есть). Подтверждено скриншотной бисекцией: на master поведение идентичное. Фикс ~3 строки: прокинуть `ballArt` (self/other из ростера) в `specOf`.
2. **Appearance: превью резервного цвета.** При клике по резервному цвету модель на фоне красится ИМ как основным (`Appearance.tsx:111-112` — «background model shows the editing color»). По новой семантике пары цветов ожидание: тело остаётся primary, резервный показывается на кольце/втором цвете модели.
3. **Appearance: устаревшая подпись.** «RESERVE COLOR (WHEN PRIMARY IS TAKEN)» / «когда основной занят» — семантика «занят» умерла в этой ветке; подпись переписать (обе локали).

- [ ] **Step 7: Обновить CHANGELOG.md и предложить мерж**
