...
export function MatchHud({ scores, matchTime, roster, localId }: MatchHudProps) {
  ...
  const display = (entry: typeof me, fallback: string) => entry ? entry.name : fallback
  return (
    <div className="match-hud">
      <div className="side you" style={{ color: me?.color }}>
        <span className="dot">●</span><span><div className="nm" data-testid="hud-name-you">{display(me, t.hudYou)}</div></span>
      ...
      <div className="side opp" style={{ color: opp?.color }}>
        ...
        <span><div className="nm" data-testid="hud-name-opp">{display(opp, t.hudOpponent)}</div></span><span className="dot">●</span>
      </div>
    </div>
  )
}
