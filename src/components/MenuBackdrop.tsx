...
import type { Pos } from './menuBallTargets'
...
// Превью анимации заряда в настройках: одноразовый прогон по клику (charge → fire → idle).
...
function signOf(balls: ActiveBall[]): string {
  return balls.map(b => `${b.key}:${b.spec.color}:${b.spec.ringColor ?? ''}:${b.spec.model}:${b.spec.windupStyle ?? ''}:${b.spec.windupSeq ?? 0}:${b.pos}:${b.slideIn ? 1 : 0}`).join('|')
}
