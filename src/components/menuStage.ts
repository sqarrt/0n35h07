...
export type MenuCameraState = 'default' | 'lobby' | 'appearance' | 'appearanceShot' | 'appearanceRespawn'
...
/** Камера-состояние по экрану и контексту: лобби вдвоём, блоки «Внешности», иначе дефолт. */
export function cameraStateFor(mode: MenuMode, hasOpponent: boolean, part: AppearancePart): MenuCameraState {
  if (mode === 'appearance') {
    if (part === 'shot') return 'appearanceShot'
    if (part === 'respawn') return 'appearanceRespawn'
    return 'appearance'
  }
  if (mode === 'lobby' && hasOpponent) return 'lobby'
  return 'default'
}
