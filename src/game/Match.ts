...
import type { IMusicEngine, MusicSection } from './audio/types'
...
export class Match {
  ...
  constructor(o: MatchOptions) {
    ...
    if (o.seedCode && o.musicEngine) this.music = new MatchMusic(o.seedCode, o.musicEngine, () => this.musicSection())
  }
  ...
  /** Музыкальная секция по состоянию матча (синхронна у обоих пиров):
   *  finale — последние MUSIC_FINALE_MS; intro — до первого убийства; иначе full. */
  private musicSection(): MusicSection {
    if (Number.isFinite(this.lastRemainingMs)) {
      const left = this.lastRemainingMs - (Date.now() - this.lastRemainingAt)
      if (left <= MUSIC_FINALE_MS) return 'finale'
    }
    return this.firstBloodDone ? 'full' : 'intro'
  }
