...
const GATHER_MS = 280              // окно возрождения: рой слетается в точку (резко — без тягучего конца)
const GATHER_LERP_BASE = 0.12      // базовая скорость слёта + рост с прогрессом
...
export class SwarmRespawnFx implements IRespawnFx {
  ...
  apply(dt: number, t: RespawnTarget, f: RespawnFrame): void {
    ...
    if (this.isRebirthActive(f.sinceRebirthMs)) {
      ...
      const pop = 1 - (1 - k) ** 2                              // ease-out: шар «вспыхивает» сразу, без тягучки
