...
const JAW_TEETH = 7                   // зубов в каждой челюсти
const JAW_WIDTH = BALL_RADIUS * 7     // размах дуги (в разы больше шара — «огромные»)
const JAW_TOOTH_LEN = BALL_RADIUS * 2.2
...
const JAW_ARCH = BALL_RADIUS * 1.1    // прогиб дуги (центр выше краёв)
const JAW_GAP = BALL_RADIUS * 0.5     // базовый зазор между челюстями (закрытая пасть)
const JAW_DISTANCE = BALL_RADIUS * 3  // насколько перед шаром висит проекция
const JAW_OPEN_DIST = BALL_RADIUS * 1.4   // ход челюсти при полном раскрытии
const JAW_OPEN_RAD = 0.35             // дополнительный развал поворотом (радианы)
...
const GLITCH_SHIFT = BALL_RADIUS * 0.35
