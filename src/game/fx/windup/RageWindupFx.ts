...
const JAW_ARCH = BALL_RADIUS * 0.75   // прогиб дуги по вертикали (центр выше краёв)
const JAW_CURVE = BALL_RADIUS * 1.2   // изгиб подковой в глубину (центр дуги вынесен по прицелу)
const JAW_EDGE_YAW = 0.6              // разворот крайних зубов вдоль дуги (радианы)
...
/** Дуга треугольных зубов. up=true — верхняя челюсть (зубы смотрят вниз). Ряд изогнут подковой:
 *  центральные зубы выдвинуты вперёд по прицелу, крайние — позади и развёрнуты наружу (как пасть). */
function buildJaw(up: boolean, mat: THREE.Material): THREE.Group {
  ...
  for (let i = 0; i < JAW_TEETH; i++) {
    ...
    const bow = 1 - (2 * t - 1) ** 2                               // парабола: 1 в центре, 0 на краях
    const arch = JAW_ARCH * bow                                    // центр выше краёв
    ...
    // Подкова: центр дуги вынесен вперёд (+Z локально — по прицелу), края отстают.
    tooth.position.set((t - 0.5) * JAW_WIDTH, (JAW_GAP / 2 + arch) * (up ? 1 : -1), JAW_CURVE * bow)
    tooth.rotation.y = -(2 * t - 1) * JAW_EDGE_YAW                 // крайние зубы развёрнуты вдоль дуги
