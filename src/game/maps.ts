...
// id is extracted from the path '../maps/<id>/<file>'. Folders not in MAP_IDS (work-in-progress maps kept on
// disk for the editor) are skipped — their artifacts must not reach the game.
const idOf = (p: string): string => p.split('/').slice(-2, -1)[0]
const isRegistered = (id: string): id is MapId => (MAP_IDS as string[]).includes(id)
