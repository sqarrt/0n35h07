...
function randomProfile(): PlayerProfile {
  ...
  return { name: pick(DEFAULT_NAMES), primaryColor, reserveColor, defaultView: 'fp', ballModel: 'smooth', windupStyle: 'classic', respawnStyle: 'echo', postProcessing: true, showFps: false, showSpeed: false, menuGlow: true, audioViz: true, volumeMaster: VOL_DEFAULT.master, volumeMusic: VOL_DEFAULT.music, volumeSfx: VOL_DEFAULT.sfx, volumeMenuMusic: VOL_DEFAULT.menuMusic, connectTimeoutSec: CONNECT_TIMEOUT_DEFAULT }
}
...
function sanitize(p: Partial<PlayerProfile>): PlayerProfile {
  ...
  return { name, primaryColor, reserveColor, defaultView, ballModel, windupStyle, respawnStyle, postProcessing, showFps, showSpeed, menuGlow, audioViz, volumeMaster, volumeMusic, volumeSfx, volumeMenuMusic, connectTimeoutSec }
}
