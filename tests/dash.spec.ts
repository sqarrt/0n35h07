...
test('Shift dash adds distance over plain W', async ({ page }) => {
  ...
  await press(page, 'ShiftLeft')   // single dash
  await page.waitForTimeout(400)
  await release(page, 'KeyW'); await release(page, 'ShiftLeft')
  const d1 = await getCameraPos(page)
  const dashed = Math.abs(d1.z - d0.z)

  expect(dashed).toBeGreaterThan(normal + 1)   // dash is noticeably farther
})

test('Shift while standing still does not move and does not spend a dash', async ({ page }) => {
  await page.goto('/'); await unlockPointer(page)
