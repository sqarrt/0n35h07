...
test('войти в лобби → ввести код → url меняется', async ({ page }) => {
  ...
  // После клика экран остаётся на join в состоянии поиска лобби (хост не отвечает)
  await expect(page.getByText('ПОИСК ЛОББИ…')).toBeVisible()
...
test('вход по несуществующему коду — таймаут показывает ошибку, ВОЙТИ снова активна', async ({ page }) => {
  ...
  await expect(page.getByText(/НЕ НАЙДЕНО/)).toBeVisible({ timeout: 13000 })
