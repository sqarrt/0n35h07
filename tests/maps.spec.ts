...
// Встать в лобби (дефолт — вкладка Матчмейкинг, host-сессия поднята; карта выбирается в «// КАРТА»).
...
test('старт на выбранной карте применяет её спавны (os_pillars)', async ({ page }) => {
  ...
  await page.getByTestId('lobby-map-os_pillars').click()   // карта на вкладке Матчмейкинг редактируема
  await page.getByTestId('lobby-tab-bot').click()          // бот в слот (выбор карты переносится в host-сессию)
