...
describe('aimPoint', () => {
  ...
  it('hit=true → ровно центр цели', () => {
    const out = aimPoint(new THREE.Vector3(), base, shooter, true, 0.5)
    expect(out.equals(base)).toBe(true)
  })

  it('near-miss: смещение ровно BALL_RADIUS*(1+grazeMargin) и перпендикулярно линии огня', () => {
    ...
    // перпендикулярно направлению shooter→base
    const dir = base.clone().sub(shooter).normalize()
    expect(offset.dot(dir)).toBeCloseTo(0, 5)
  })

  it('меньший grazeMargin (сильный бот) → ближе к центру', () => {
  ...
  it('разный угол rng → разные точки на окружности промаха', () => {
